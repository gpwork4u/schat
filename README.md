...
UI 有**兩種獨立運行模式**（plugin 與 server 解耦，互不依賴）：
- **內建模式（免 server）**：UI 打包進 extension，以 `chrome-extension://` 分頁開啟，直接用 `chrome.runtime` 接 background。適合日常使用。
- **開發模式（HMR）**：UI 跑在 `localhost:5173`（Vite），透過 `app-bridge` content script 橋接。適合改 code。

### 1. 載入 extension
1. （內建模式需要）先 build 一次 UI：
   ```bash
   cd web && npm install && npm run build:ext   # → 產出 extension/app/
   ```
2. Chrome → `chrome://extensions` → 右上開「**開發者模式**」→「**載入未封裝項目**」→ 選 `extension/` 目錄。

### 2. 開啟並登入 Google Chat
開分頁到 `https://chat.google.com/`，登入後**點開任一對話**（讓 extension 學到 session 的 xsrf / batchexecute 參數）。

### 3. 開啟 Schat UI
點 extension popup：
- **開啟 Schat（內建，免 server）** → 直接開內建 UI 分頁。
- **開啟 localhost 開發版** → 需先 `cd web && npm run dev`（`localhost:5173`，有 HMR）。

> **改 `web/*`**：開發模式 Vite HMR 自動生效；內建模式需重跑 `npm run build:ext` + reload extension。
> **改 `extension/*`** → `chrome://extensions` 按 ↻，並在 chat 分頁 `Cmd/Ctrl+Shift+R` 強制重新注入。
>
> 註：**內建模式**下，直接呼叫外部 AI provider（OpenAI/Anthropic）的 fetch 受 `host_permissions` 限制；如需在內建分頁用「直連 provider」的 AI 自動回覆，需另加該網域權限（chat 操作本身不受影響）。
